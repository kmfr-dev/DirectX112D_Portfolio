...
protected:
	...
	CSharedPtr<class CMainWidget> mMainWidget;

	CSharedPtr<class CTileMapObj>	mMapObj;
	std::list<CSharedPtr<class CSceneObject>>	mObjList;

	bool	mDebugQuadTree = true;

public:
	class CTileMapObj* GetMapObj() const
	{
		return mMapObj;
	}
...
public:
	...
	class CNPCObject* FindNPCByID(int NPCID);
};